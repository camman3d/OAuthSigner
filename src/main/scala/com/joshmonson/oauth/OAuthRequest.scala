 ...
 *
 * [[http://oauth.net/core/1.0a/ OAuth 1.0a Specification]]
 * [[http://oauth.googlecode.com/svn/spec/ext/body_hash/1.0/drafts/4/spec.html Body Hash Extension Specification]]
 *
 * This is HTTP library agnostic, which means it will work with anything, but you'll have to set the Authorization
 * manually.
 *
 * ==Usage==
 * To sign a request, create an OAuthRequest object and sign it with an [[com.joshmonson.oauth.OAuthKey]] object:
 * {{{
...
case class OAuthRequest(
  ...
  def verify(implicit key: OAuthKey): Boolean = {
    ...
    if (signaturePackage._5.isDefined) {
      println("Generated signature: " + signaturePackage._1)
      println("Given signature: " + signaturePackage._5.get)
    }
